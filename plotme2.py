...
fil2=open("./histogram"+sample_number+"/"+"plotthis2.py","w")
...
for infile in glob.glob( os.path.join(path, 'ncbi*.psl') ):
	...
	for line in fileinput.FileInput("./histogram"+sample_number+"/"+infile[len(path)+1:len(infile)], inplace=1):
		...
		if (count>5):
			...
			if(int(line.split("\t")[17])==1): #block size, if it is one then draw one second line 
				...
				fil2.write("plt.plot(x1, y1,color='purple', marker = 's',linewidth=15.0)\n")
